...
// --- CONFIGURAZIONE ---
// Password in chiaro per garantire funzionamento su HTTP locale (mobile)
...
const App = () => {
    ...
    const [postForm, setPostForm] = useState({ 
        id: null, title: '', type: 'gaffe', content: '', date: '', source: '', body: '', image: '' 
    });
    
    // Politician Form State
    const [profileForm, setProfileForm] = useState({ 
        id: '', name: '', party: '', role: '', avatar: '', bio: '', banner: 'bg-gray-500' 
    });
    ...
    const handleSavePoliticianProfile = () => {
        ...
        if (newPoliticianMode) {
            ...
            setDb({ ...db, politicians: [...db.politicians, newPol] });
            handleSelectPolitician(newPol);
        } else {
            const updatedPol = { ...editingPolitician, ...profileForm };
            updatePoliticianInDb(updatedPol);
        }
    ...
    const handleDeletePolitician = (e, politicianId) => {
        e.stopPropagation();
        if (!window.confirm("ATTENZIONE: Stai per eliminare questo politico e tutti i suoi dati. Continuare?")) return;
    ...
    const updatePoliticianInDb = (updatedPolitician) => {
        ...
        if (selectedProfile && selectedProfile.id === updatedPolitician.id) {
        ...
        <div className="flex h-screen bg-[#F4F1EA] text-[#1a1a1a] overflow-hidden">
            ...
            <div className="hidden md:flex flex-col w-64 bg-[#F4F1EA] border-r border-black p-0 z-10">
                <div className="p-4 border-b border-black bg-yellow-400">
                    <div className="text-2xl font-black tracking-tighter flex items-center gap-2 serif-font uppercase">
                        <AlertTriangle className="text-black stroke-[3]" size={24} />
                        Politi<br/>Gaffe
                    </div>
                </div>
                
                <nav className="flex-1 p-3 space-y-2">
                    <div className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2 px-1">Navigazione</div>
                    <button onClick={() => setView('home')} className={`w-full flex items-center gap-3 px-3 py-2 border border-black text-sm transition-all shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] ${view === 'home' ? 'bg-black text-white' : 'bg-white text-black'}`}>
                        <Home size={16} /> <span className="font-bold">Dashboard</span>
                    </button>
                    <button onClick={() => setView('about')} className={`w-full flex items-center gap-3 px-3 py-2 border border-black text-sm transition-all shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] ${view === 'about' ? 'bg-black text-white' : 'bg-white text-black'}`}>
                        <Info size={16} /> <span className="font-bold">Manifesto</span>
                    </button>
                </nav>
                
                <div className="p-4 border-t border-black bg-white flex justify-between items-center">
                    <p className="text-[10px] leading-relaxed font-mono text-gray-500">
                        v.1.8 - Fix Mobile Login
                    </p>
                    <button 
                    ...
                    >
                        {isAdmin ? <Settings size={14} /> : <Lock size={14} />}
                    </button>
                </div>
            </div>
            ...
            {mobileMenuOpen && (
                <div className="fixed inset-0 z-[60] bg-[#F4F1EA] flex flex-col p-6 animate-in slide-in-from-right duration-200">
                    <div className="flex justify-between items-center mb-8 border-b border-black pb-4">
                        <div className="text-xl font-black uppercase serif-font flex items-center gap-2">
                             <AlertTriangle size={20} /> Menu
                        </div>
                        <button onClick={() => setMobileMenuOpen(false)} className="p-2 border border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none">
                            <X size={24} />
                        </button>
                    </div>

                    <nav className="flex-1 space-y-4">
                        <button onClick={() => navigateTo('home')} className="w-full text-left p-4 border border-black bg-white text-lg font-bold uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all flex items-center gap-4">
                            <Home size={24} /> Dashboard
                        </button>
                        <button onClick={() => navigateTo('about')} className="w-full text-left p-4 border border-black bg-white text-lg font-bold uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all flex items-center gap-4">
                            <Info size={24} /> Manifesto
                        </button>
                    </nav>
                    ...
                    <div className="mt-auto pt-6 border-t border-black">
                        <button 
                            ...
                            className="w-full flex items-center justify-center gap-2 p-3 bg-gray-200 text-gray-700 font-bold uppercase text-sm border border-gray-400"
                        >
            ...
            <main className="flex-1 overflow-y-auto relative w-full">
                ...
                <div className="md:hidden sticky top-0 z-50 bg-yellow-400 border-b border-black p-3 flex justify-between items-center shadow-md h-16">
                    <div className="w-10">
                        {view !== 'home' ? (
                            <button onClick={() => view === 'article' ? goBackToProfile() : setView('home')} className="p-1 border border-black bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none">
                                <ArrowLeft size={20} />
                            </button>
                        ) : (
                            <button onClick={() => setView('login')} className="p-1 text-black opacity-50">
                                <Lock size={16} />
                            </button>
                        )}
                    </div>

                    <div className="font-black text-xl flex items-center gap-2 serif-font uppercase tracking-tighter">
                        <AlertTriangle className="text-black stroke-[2]" size={24} /> PolitiGaffe
                    </div>

                    <div className="w-10 flex justify-end">
                        <button onClick={() => setMobileMenuOpen(true)} className="p-1 border border-black bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none">
                            <Menu size={20} />
                        </button>
                    </div>
                </div>
                ...
                {view === 'home' && (
                    <div className="max-w-5xl mx-auto p-4 md:p-8 pb-24">
                        <header className="mb-8 pt-4 border-b-2 border-black pb-6">
                            <h1 className="text-3xl md:text-5xl font-black text-black mb-3 serif-font uppercase tracking-tight">
                                Osservatorio Politico
                            </h1>
                            <p className="text-sm md:text-base text-gray-700 max-w-3xl font-medium leading-relaxed font-serif">
                                Monitoraggio indipendente di dichiarazioni pubbliche. <br/>
                                <span className="italic text-gray-500">Lo scopo è stimolare il pensiero critico e la memoria collettiva.</span>
                            </p>
                        </header>
                        ...
                        <div className="mb-8 flex items-center border border-black bg-white p-1 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] max-w-lg">
                            ...
                            <input 
                                ...
                                className="w-full pl-3 pr-3 py-2 bg-transparent focus:outline-none text-sm font-bold placeholder-gray-400 uppercase"
                                value={searchTerm}
                            ...
                            {filteredPoliticians.map(p => (
                                <div key={p.id} onClick={() => handleProfileClick(p)} className="group cursor-pointer bg-white border border-black p-0 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-all">
                                    <div className="flex border-b border-black">
                                        <div className="w-24 h-24 border-r border-black bg-gray-100 shrink-0 overflow-hidden">
                                            <img src={getImageUrl(p.avatar)} alt={p.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-300" />
                                        </div>
                    ...
                    <div className="min-h-full bg-white animate-in slide-in-from-right duration-300">
                        ...
                        <div className="max-w-6xl mx-auto px-4 md:px-6 -mt-8 mb-12 relative">
                            ...
                            <div className="flex flex-col md:flex-row gap-5 items-end mb-6">
                                <div className="w-28 h-28 border-2 border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] shrink-0 overflow-hidden">
                                    <img src={getImageUrl(selectedProfile.avatar)} alt="Profile" className="w-full h-full object-cover" />
                                </div>
                                <div className="flex-1 pb-1">
                                    <h1 className="text-3xl font-black serif-font mb-1 uppercase leading-none">{selectedProfile.name}</h1>
                                    <p className="text-sm font-serif italic text-gray-600 mb-2 border-l-2 border-yellow-400 pl-2">
                                        "{selectedProfile.bio}"
                                    </p>
                            ...
                            {/* Statistiche Ridotte */}
                            <div className="grid grid-cols-3 border border-black bg-[#F4F1EA] mb-6 text-center text-xs">
                                <div className="p-2 border-r border-black">
                                    <div className="uppercase tracking-wide text-gray-500 mb-0.5 text-[10px]">Followers</div>
                                    <div className="font-bold">{selectedProfile.stats.followers}</div>
                                </div>
                                <div className="p-2 border-r border-black">
                                    <div className="uppercase tracking-wide text-gray-500 mb-0.5 text-[10px]">Gaffe</div>
                                    <div className="font-bold text-red-600">{selectedProfile.stats.gaffes}</div>
                                </div>
                                <div className="p-2">
                                    <div className="uppercase tracking-wide text-gray-500 mb-0.5 text-[10px]">Voltafaccia</div>
                                    <div className="font-bold text-blue-600">{selectedProfile.stats.incoherences}</div>
                                </div>
                            </div>

                            {/* Tabella Coerenza Compact */}
                            {selectedProfile.inconsistencies.length > 0 && (
                                <div className="border border-black bg-white shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] mb-8">
                                    <div className="bg-black text-white px-3 py-1.5 font-bold uppercase text-xs flex items-center gap-2">
                                        <ArrowRightLeft size={14} /> Analisi Coerenza
                                    </div>
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-xs text-left border-collapse">
                                            <thead>
                                                <tr className="border-b border-black font-mono uppercase bg-gray-100 text-[10px]">
                                                    <th className="p-2 border-r border-black w-1/4">Tema</th>
                                                    <th className="p-2 border-r border-black w-1/3 text-gray-500">Ieri (Opposizione)</th>
                                                    <th className="p-2 w-1/3">Oggi (Governo)</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {selectedProfile.inconsistencies.map((inc, idx) => (
                                                    <tr key={idx} className="border-b border-black last:border-0 hover:bg-yellow-50">
                                                        <td className="p-2 border-r border-black font-bold bg-gray-50">{inc.topic}</td>
                                                        <td className="p-2 border-r border-black italic text-gray-600 font-serif">"{inc.then}"</td>
                                                        <td className="p-2 font-bold bg-yellow-100/30">"{inc.now}"</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                            ...
                            {/* DUE COLONNE: DOSSIER vs CRONACA */}
                            <div className="grid md:grid-cols-2 gap-8">
                                
                                {/* COLONNA 1: REGISTRO DOSSIER (GAFFE) */}
                                <div>
                                    <div className="flex items-center justify-between border-b border-black pb-2 mb-4">
                                        <div className="flex items-center gap-2 font-bold uppercase text-sm">
                                            <FileText size={16} /> Registro Dossier
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        {visibleGaffes.length > 0 ? visibleGaffes.map(post => (
                                            <div key={post.id} onClick={() => handlePostClick(post)} className="cursor-pointer border border-black bg-white p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-black hover:text-white group transition-colors">
                                                <div className="flex justify-between items-center mb-1">
                                                    <span className={`text-[9px] font-bold uppercase px-1 border border-black group-hover:border-white ${
                                                        post.type === 'gaffe' ? 'bg-red-500 text-white' : 'bg-purple-500 text-white'
                                                    }`}>
                                                        {post.type}
                                                    </span>
                                                    <span className="text-[9px] font-mono group-hover:text-gray-300">{post.date}</span>
                                                </div>
                                        ...
                                        {!showFullRegister && gaffePosts.length > 5 && (
                                            <button onClick={() => setShowFullRegister(true)} className="w-full py-2 text-xs font-bold uppercase border border-black border-dashed hover:bg-gray-100">
                                                Vedi registro completo ({gaffePosts.length})
                                            </button>
                                        )}
                                        {showFullRegister && (
                                            <button onClick={() => setShowFullRegister(false)} className="w-full py-2 text-xs font-bold uppercase border border-black border-dashed hover:bg-gray-100">
                                                Riduci
                                            </button>
                                        )}
                                    </div>
                                ...
                                {/* COLONNA 2: CRONACA & NEWS */}
                                <div>
                                    <div className="flex items-center gap-2 font-bold uppercase text-sm border-b border-black pb-2 mb-4">
                                        <Newspaper size={16} /> Notizie & Cronaca
                                    </div>
                                    <p className="text-xs text-gray-500 mb-4 italic">Cronaca pura e gratuita, per contestualizzare.</p>
                                    ...
                                    <div className="space-y-4">
                                        {newsPosts.length > 0 ? newsPosts.map(post => (
                                            <div key={post.id} onClick={() => handlePostClick(post)} className="cursor-pointer group flex gap-3 items-start border-b border-gray-300 pb-3 last:border-0 hover:bg-gray-50 p-1">
                                                <div className="w-16 h-16 bg-gray-200 shrink-0 border border-black overflow-hidden">
                                                    <img src={getImageUrl(post.image)} className="w-full h-full object-cover" alt="news" />
                                                </div>
                                                <div>
                                                    <div className="flex gap-2 text-[9px] font-bold uppercase text-blue-700 mb-0.5">
                                                        <span>{post.source}</span>
                                                        <span className="text-gray-400">• {post.date}</span>
                                                    </div>
                                                    <h4 className="font-serif font-bold text-sm leading-tight mb-1 group-hover:underline">{post.title}</h4>
                                                    <p className="text-[10px] text-gray-600 line-clamp-2">{post.content}</p>
                                                </div>
                                            </div>
                                        )) : (
                                            <div className="p-4 text-center border border-dashed border-gray-400 text-xs text-gray-500">
                                                Nessuna notizia recente in cronaca.
                                            </div>
                                        )}
                                    </div>
                                </div>
                ...
                {view === 'login' && (
                    <div className="flex items-center justify-center min-h-screen bg-gray-100">
                        <div className="bg-white p-8 border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-full max-w-sm">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-xs font-bold uppercase mb-1">Password Admin</label>
                                    <input 
                                        type="password" 
                                        value={passwordInput}
                                        onChange={(e) => setPasswordInput(e.target.value)}
                                        onKeyDown={(e) => e.key === 'Enter' && handleLogin()}
                                        className="w-full p-2 border border-black text-sm outline-none focus:bg-yellow-50"
                                        placeholder="Inserisci password..."
                                    />
                                    {loginError && <p className="text-red-600 text-xs mt-1 font-bold">Password errata. Riprova.</p>}
                                </div>
                ...
                {view === 'admin' && (
                    <div className="max-w-6xl mx-auto p-4 md:p-6 bg-white min-h-full">
                        <header className="mb-8 pb-4 border-b border-black flex justify-between items-center">
                            <h1 className="text-2xl md:text-3xl font-black uppercase serif-font">Pannello Admin</h1>
                            <button onClick={handleLogout} className="flex items-center gap-1 text-xs uppercase font-bold text-red-600 hover:text-red-800">
                                <LogOut size={14}/> Esci
                            </button>
                        </header>
                            ...
                            <div className="border border-black p-4 bg-gray-50 h-[80vh] flex flex-col">
                                ...
                                <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
                                    {db.politicians.map(p => (
                                        <div 
                                            key={p.id} 
                                            className={`p-2 border border-black flex justify-between items-center text-xs font-bold cursor-pointer ${editingPolitician?.id === p.id ? 'bg-black text-white' : 'bg-white hover:bg-gray-200'}`}
                                            onClick={() => handleSelectPolitician(p)}
                                        >
                                            <div className="flex items-center gap-2">
                                                <img src={getImageUrl(p.avatar)} className="w-6 h-6 rounded-full bg-gray-200 object-cover border border-gray-400" />
                                                <span>{p.name}</span>
                                            </div>
                                            <div className="flex gap-2">
                                                <button 
                                                    onClick={(e) => handleDeletePolitician(e, p.id)} 
                                                    className={`hover:text-red-500 ${editingPolitician?.id === p.id ? 'text-gray-400' : 'text-gray-400'}`}
                                                    title="Elimina Politico"
                                                >
                                                    <Trash2 size={14}/>
                                                </button>
                                            </div>
                                ...
                                {editingPolitician ? (
                                    <>
                                        <div className="flex justify-between items-center border-b border-black pb-4 mb-4">
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => setEditMode('posts')} className={`px-3 py-1 text-xs font-bold uppercase border border-black ${editMode === 'posts' ? 'bg-black text-white' : 'bg-white'}`}>Gestione Post</button>
                                                <button onClick={() => setEditMode('profile')} className={`px-3 py-1 text-xs font-bold uppercase border border-black ${editMode === 'profile' ? 'bg-black text-white' : 'bg-white'}`}>Dati Profilo</button>
                                            </div>
                                        </div>
                                            ...
                                            <div className="bg-gray-50 p-4 border border-black">
                                                <div className="grid grid-cols-2 gap-4 mb-4">
                                                    <div className="space-y-1">
                                                        ...
                                                        <input className="w-full p-2 border border-black text-sm" value={profileForm.name} onChange={e => setProfileForm({...profileForm, name: e.target.value})} />
                                                    </div>
                                                    <div className="space-y-1">
                                                        <label className="text-[10px] font-bold uppercase">Partito</label>
                                                        <input className="w-full p-2 border border-black text-sm" value={profileForm.party} onChange={e => setProfileForm({...profileForm, party: e.target.value})} />
                                                    </div>
                                                    <div className="space-y-1">
                                                        <label className="text-[10px] font-bold uppercase">Ruolo</label>
                                                        <input className="w-full p-2 border border-black text-sm" value={profileForm.role} onChange={e => setProfileForm({...profileForm, role: e.target.value})} />
                                                    </div>
                                                    <div className="space-y-1">
                                                        <label className="text-[10px] font-bold uppercase">Avatar URL</label>
                                                        <input className="w-full p-2 border border-black text-sm" value={profileForm.avatar} onChange={e => setProfileForm({...profileForm, avatar: e.target.value})} />
                                                    </div>
                                                </div>
                                                <div className="space-y-1 mb-4">
                                                    <label className="text-[10px] font-bold uppercase">Bio</label>
                                                    <textarea className="w-full p-2 border border-black text-sm h-20" value={profileForm.bio} onChange={e => setProfileForm({...profileForm, bio: e.target.value})} />
                                                </div>
                                                <button onClick={handleSavePoliticianProfile} className="w-full bg-green-600 text-white p-2 text-sm font-bold uppercase flex items-center justify-center gap-2"><Save size={16}/> Salva Profilo</button>
                                            </div>
                                        )}
                                            ...
                                            <div className="flex flex-col h-full">
                                                ...
                                                <div className={`p-4 border border-black mb-6 ${postForm.id ? 'bg-blue-50' : 'bg-yellow-50'}`}>
                                                    <div className="flex justify-between items-center mb-2">
                                                        <h4 className="font-bold text-xs uppercase">{postForm.id ? 'Modifica Post' : 'Nuovo Post'}</h4>
                                                        {postForm.id && <button onClick={resetPostForm} className="text-[10px] underline">Annulla Modifica</button>}
                                                    </div>
                                                    <div className="space-y-2">
                                                        ...
                                                        <textarea className="w-full p-2 border border-gray-300 text-sm h-20" placeholder="Contenuto (Breve descrizione)" value={postForm.content} onChange={e => setPostForm({...postForm, content: e.target.value})} />
                                                        <textarea className="w-full p-2 border border-gray-300 text-sm h-20" placeholder="Body (Testo completo articolo)" value={postForm.body} onChange={e => setPostForm({...postForm, body: e.target.value})} />
                                                        <div className="flex gap-2">
                                                            <select className="p-2 border text-sm" value={postForm.type} onChange={e => setPostForm({...postForm, type: e.target.value})}>
                                                                <option value="gaffe">Gaffe</option>
                                                                <option value="quote">Incoerenza</option>
                                                                <option value="news">News</option>
                                                            </select>
                                                            <input className="flex-1 p-2 border text-sm" placeholder="Data (es. 21 Gen 2020)" value={postForm.date} onChange={e => setPostForm({...postForm, date: e.target.value})} />
                                                        </div>
                                                        ...
                                                        <input className="w-full p-2 border border-gray-300 text-sm" placeholder="URL Immagine (Drive o Web)" value={postForm.image} onChange={e => setPostForm({...postForm, image: e.target.value})} />
                                                        <button onClick={handleSavePost} className="w-full bg-black text-white p-2 text-sm font-bold uppercase hover:bg-gray-800">
                                                            {postForm.id ? 'Aggiorna Post' : 'Pubblica Post'}
                                                        </button>
                                                    </div>
                                                </div>

                                                <h4 className="font-bold text-xs uppercase mb-2">Storico Post</h4>
                                                <div className="flex-1 overflow-y-auto space-y-1 custom-scrollbar max-h-64 border-t border-black pt-2">
                                                    {editingPolitician.posts.length > 0 ? editingPolitician.posts.map(post => (
                                                        <div key={post.id} className="flex justify-between items-center p-2 bg-gray-100 border border-gray-300 text-xs">
                                                            ...
                                                            <span className="text-[10px] uppercase text-gray-500 w-1/4">{post.type}</span>
                                                            <div className="flex gap-2">
                                                                <button onClick={() => handleEditPostClick(post)} className="text-blue-600 hover:text-blue-800" title="Modifica"><Edit3 size={14}/></button>
                                                                <button onClick={() => handleDeletePost(post.id)} className="text-red-600 hover:text-red-800" title="Elimina"><Trash2 size={14}/></button>
                                                            </div>
                                                        </div>
                                    ...
                                    <div className="h-full flex items-center justify-center text-gray-400 text-xs uppercase">
                                        <div className="text-center">
                                            <Settings size={48} className="mx-auto mb-2 opacity-20"/>
                                            Seleziona un politico per iniziare
                                        </div>
                                    </div>
                                )}
                            </div>
                ...
                {view === 'article' && selectedPost && (
                    <div className="bg-[#F4F1EA] min-h-full animate-in fade-in duration-200">
                         <div className="sticky top-0 bg-[#F4F1EA] border-b border-black px-4 py-3 flex justify-between items-center z-50">
                            <button onClick={goBackToProfile} className="flex items-center gap-2 font-bold uppercase hover:underline text-xs">
                        ...
                        <article className="max-w-2xl mx-auto px-6 py-10 bg-white border-x border-black min-h-screen shadow-xl">
                            <header className="mb-6 text-center border-b border-black pb-6">
                                <div className={`inline-block px-2 py-0.5 text-white font-mono text-[10px] font-bold uppercase mb-2 ${selectedPost.type === 'news' ? 'bg-blue-600' : 'bg-red-600'}`}>
                                    {selectedPost.type === 'news' ? 'Cronaca' : 'Dossier Gaffe'}
                                </div>
                                <h1 className="text-3xl font-black serif-font mb-2 leading-tight">
                                    {selectedPost.title}
                                </h1>
                                <div className="text-xs font-mono text-gray-500 uppercase">
                                    {selectedPost.date} • Fonte: {selectedPost.source}
                                </div>
                            </header>

                            {selectedPost.image && (
                                <div className="border border-black p-1 mb-8 bg-gray-100">
                                    <div className="aspect-video w-full bg-gray-300 overflow-hidden grayscale contrast-125">
                                        <img src={getImageUrl(selectedPost.image)} alt={selectedPost.title} className="w-full h-full object-cover" />
                                    </div>
                                </div>
                            )}

                            <div className="prose prose-sm prose-slate mx-auto font-serif text-black leading-loose">
                                <p className="text-lg font-bold leading-relaxed mb-6 border-l-2 border-yellow-400 pl-3 italic">
