...
import { Home, Info, AlertTriangle, Search, ArrowLeft, ArrowRightLeft, FileText, Eye, ChevronRight, Settings, Trash2, Newspaper, Lock, LogOut, Printer, Menu, X, Edit3, Save, PlusCircle, Loader2 } from 'lucide-react';
...
// Inizializza Firebase solo se c'è una config valida
let dbFire;
try {
    const app = initializeApp(firebaseConfig);
    dbFire = getFirestore(app);
} catch (e) {
    console.error("Firebase non configurato o errore init:", e);
}

// Password Admin (semplice per mobile)
...
// --- DATI DI BACKUP (Se Firebase è vuoto o offline) ---
const backupDb = {
    politicians: [
        {
            id: 'demo', name: 'Politico Demo (Locale)', party: 'Test', role: 'Esempio',
            avatar: '', banner: 'bg-gray-500', bio: "Dati locali visualizzati perché il database è vuoto.",
            stats: { followers: '0', gaffes: 0, incoherences: 0 },
...
const App = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        if (!dbFire) {
            setPoliticians(backupDb.politicians); // Fallback se no config
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            if (data.length === 0) {
                // Se il DB è vuoto, usa il backup (o potresti caricare initialDb su Firebase una volta)
                setPoliticians(backupDb.politicians); 
            } else {
                setPoliticians(data);
            }
        } catch (error) {
    ...
    const handleSavePoliticianProfile = async () => {
        ...
        try {
            ...
            if (newPoliticianMode) {
                ...
                setEditingPolitician(polData); // Refresh edit view
            }
    ...
    // --- UI HELPERS (Invariati) ---
    const handleSelectPolitician = (p) => {
        setEditingPolitician(p); setProfileForm(p); setEditMode('posts'); setNewPoliticianMode(false); resetPostForm();
    };
    const resetPostForm = () => setPostForm({ id: null, title: '', type: 'gaffe', content: '', date: '', source: '', body: '', image: '' });
    const setupNewPolitician = () => {
        setNewPoliticianMode(true); setEditingPolitician({ id: 'new', name: 'Nuovo Politico', posts: [] }); 
        setProfileForm({ id: '', name: '', party: '', role: '', avatar: '', bio: '', banner: 'bg-gray-500' }); setEditMode('profile');
    };
        ...
        <div className="flex h-screen bg-[#F4F1EA] text-[#1a1a1a] overflow-hidden">
            ...
            <div className="hidden md:flex flex-col w-64 bg-[#F4F1EA] border-r border-black p-0 z-10">
                <div className="p-4 border-b border-black bg-yellow-400">
                    <div className="text-2xl font-black tracking-tighter flex items-center gap-2 serif-font uppercase">
                        <AlertTriangle className="text-black stroke-[3]" size={24} /> Politi<br/>Gaffe
                    </div>
                </div>
                <nav className="flex-1 p-3 space-y-2">
                    <div className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2 px-1">Navigazione</div>
                    <button onClick={() => setView('home')} className={`w-full flex items-center gap-3 px-3 py-2 border border-black text-sm transition-all shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] ${view === 'home' ? 'bg-black text-white' : 'bg-white text-black'}`}>
                        <Home size={16} /> <span className="font-bold">Dashboard</span>
                    </button>
                    <button onClick={() => setView('about')} className={`w-full flex items-center gap-3 px-3 py-2 border border-black text-sm transition-all shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] ${view === 'about' ? 'bg-black text-white' : 'bg-white text-black'}`}>
                ...
                <div className="p-4 border-t border-black bg-white flex justify-between items-center">
                    <p className="text-[10px] leading-relaxed font-mono text-gray-500">v.2.0 - Cloud DB</p>
            ...
            <main className="flex-1 overflow-y-auto relative w-full">
                ...
                <div className="md:hidden sticky top-0 z-50 bg-yellow-400 border-b border-black p-3 flex justify-between items-center shadow-md h-16">
                    <div className="w-10">
                        {view !== 'home' ? (
                            <button onClick={() => view === 'article' ? goBackToProfile() : setView('home')} className="p-1 border border-black bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none"><ArrowLeft size={20} /></button>
                        ) : (
                            <button onClick={() => setView('login')} className="p-1 text-black opacity-50"><Lock size={16} /></button>
                        )}
                    </div>
                    ...
                    <div className="w-10 flex justify-end">
                        <button onClick={() => setMobileMenuOpen(true)} className="p-1 border border-black bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none"><Menu size={20} /></button>
                    </div>
                </div>
                    ...
                    <div className="max-w-5xl mx-auto p-4 md:p-8 pb-24">
                        <header className="mb-8 pt-4 border-b-2 border-black pb-6">
                            ...
                            <p className="text-sm md:text-base text-gray-700 max-w-3xl font-medium leading-relaxed font-serif">
                                Database in Cloud. {dbFire ? <span className="text-green-600 font-bold">● Online</span> : <span className="text-red-600 font-bold">● Offline (Modalità Demo)</span>}
                            </p>
                        </header>
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                <div>
                                    <div className="flex items-center justify-between border-b border-black pb-2 mb-4">
                                        <div className="flex items-center gap-2 font-bold uppercase text-sm"><FileText size={16} /> Registro Dossier</div>
                                    </div>
                                        ...
                                        {visibleGaffes.length > 0 ? visibleGaffes.map(post => (
                                            <div key={post.id} onClick={() => handlePostClick(post)} className="cursor-pointer border border-black bg-white p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-black hover:text-white group transition-colors">
                                                <div className="flex justify-between items-center mb-1">
                                                    <span className={`text-[9px] font-bold uppercase px-1 border border-black group-hover:border-white ${post.type === 'gaffe' ? 'bg-red-500 text-white' : 'bg-purple-500 text-white'}`}>{post.type}</span>
                                    ...
                                    <div className="space-y-4">
                                        {newsPosts.length > 0 ? newsPosts.map(post => (
                                            <div key={post.id} onClick={() => handlePostClick(post)} className="cursor-pointer group flex gap-3 items-start border-b border-gray-300 pb-3 last:border-0 hover:bg-gray-50 p-1">
                                                ...
                                                <div>
                                                    <div className="flex gap-2 text-[9px] font-bold uppercase text-blue-700 mb-0.5"><span>{post.source}</span><span className="text-gray-400">• {post.date}</span></div>
